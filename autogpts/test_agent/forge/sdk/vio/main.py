...
def get_agent_task(task_id: str):
    task = next((item for item in mock_db['General Challenge']['tasks'] if item["task_id"] == task_id), None)
    return {"task": task or {}}
...
def download_agent_task_artifact(task_id: str, artifact_id: str):
    task = next((item for item in mock_db['General Challenge']['tasks'] if item["task_id"] == task_id), None)
    if task:
        artifact = next((a for a in task["artifacts"] if a["artifact_id"] == artifact_id), None)
